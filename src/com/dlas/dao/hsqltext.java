...
public class hsqltext {
	...
	public String insertmvtnum ()
	{
		...
		sqlstmt = "INSERT INTO PUBLIC.MVT_NUM (MVT_NUM_ID,STRUCTURE_NAME, CERTIFICATE, CERTIFICATE_START, CERTIFICATE_RENEWAL, CERTIFICATE_CANCELLATION, AMENDMENT_START, AMENDMENT_END, WYCC_ID,";
		...
		sqlstmt = sqlstmt+" SELECT NULL,STRUCTURE_NAME,CERTIFICATE,CERTIFICATE_START,CERTIFICATE_RENEWAL,CERTIFICATE_CANCELLATION,AMENDMENT_START,AMENDMENT_END,";
	...
	public String insertbeneficiairies ()
	{
        ...
        sqlstmt = " INSERT INTO PUBLIC.BENEFICIAIRIES_TAB (BENEFICIAIRIES_ID,WYCC_ID, NAME, FIRST_NAME, LINE, START_MOVEMENT, PREVMVT, ENDCOMP, END_MOVEMENT, NEXTMVT, NEXTCOMP, MONTHLY_SALARY, DRESTEJ, ERESTEJ, JOUR, TO_INVOICE)";
		sqlstmt = sqlstmt+" SELECT";
		sqlstmt = sqlstmt+" 	TO_CHAR(LINE,'000000')||WYCC_ID BENEFICIAIRIS_ID,WYCC_ID, NAME, FIRST_NAME,  LINE,";
		sqlstmt = sqlstmt+"     START_MOVEMENT,";
		sqlstmt = sqlstmt+"     PREVMVT,";
		sqlstmt = sqlstmt+"     DATEDIFF('DAY', START_MOVEMENT, DATEADD('DAY', -1, PARSEDATETIME(FORMATDATETIME(PREVMVT, 'dd/MM/yyyy hh:mm:ss'), 'dd/MM/yyyy'))) AS ENDCOMP,";
		sqlstmt = sqlstmt+"     END_MOVEMENT,";
		sqlstmt = sqlstmt+"     NEXTMVT,";
		sqlstmt = sqlstmt+"     DATEDIFF('DAY', END_MOVEMENT, DATEADD('DAY', -1, PARSEDATETIME(FORMATDATETIME(NEXTMVT, 'dd/MM/yyyy hh:mm:ss'), 'dd/MM/yyyy'))) AS NEXTCOMP,";
		sqlstmt = sqlstmt+"     MONTHLY_SALARY,";
		sqlstmt = sqlstmt+"     DRESTEJ, ERESTEJ, JOUR, ((CASE WHEN (LINE = 1) THEN (MOIS + 1) ELSE MOIS END + CASE WHEN ((LINE <> 1) AND (DATEDIFF('DAY', START_MOVEMENT, DATEADD('DAY', 1, PARSEDATETIME(FORMATDATETIME(PREVMVT, 'dd/MM/yyyy hh:mm:ss'), 'dd/MM/yyyy'))) = 0)) THEN ROUND(((30 - DRESTEJ) / 30.0), 5) ELSE 0 END) + CASE WHEN ((LINE <> 1)  AND (DATEDIFF('DAY', END_MOVEMENT, DATEADD('DAY', -1, PARSEDATETIME(FORMATDATETIME(NEXTMVT, 'dd/MM/yyyy hh:mm:ss'), 'dd/MM/yyyy'))) = 0)) THEN ROUND((ERESTEJ / 30.0), 5) ELSE 0 END) AS TO_INVOICE";
		sqlstmt = sqlstmt+" FROM PUBLIC.LISTMVT";
		return 	sqlstmt;
	}
