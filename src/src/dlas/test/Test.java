...
public class Test {
	...
	/*
	 * public static void main(String arg[]) { String retour = ""; retour =
	 * getNewNumColonne("AS5-AU5", "[$,A-Z]*", 26, 1 * 57);
	 * 
	 * retour =getNewNumColonne("AA5*(1+AB5)","[$,A-Z]*",26,2*57); retour
	 * =getNewNumColonne("AE5/12*$Q$5","[$,A-Z]*",26,2*57); retour
	 * =getNewNumColonne("EI5*$O$5*12","[$,A-Z]*",26,2*57); retour
	 * =getNewNumColonne("BE14*BC14","[$,A-Z]*",26,2*57);
	 * 
	 * 
	 * }
	 */
		...
		while (m.find()) {
			if (!m.group().startsWith("$")) {
				if (m.group().length() != 0) {
					 ...
					 * 
					 * } else { colonenumber=64+(colOffset+colInt); }
					 */
